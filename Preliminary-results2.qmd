...
# Print the table
print(mean_family_sugar)

# Optional: Display a clean table for the Quarto output
knitr::kable(mean_family_sugar, caption = "Mean Sugar Concentration per Family")
############
...
family_names <- mean_family_sugar$Family  # Replace with your actual dataframe name
...
ggtree(tree, size = 0.7) +  # Reduce branch thickness
  ...
  theme_tree2() +
  scale_x_continuous(expand = c(0, 0.5))  # Adjust X-axis space to fit labels
...
# Step 1: Prepare the sugar concentration data
# Filter and compute mean sugar concentration per family
sugar_data <- bee_data %>%
  filter(!is.na(Family) & Family != "", !is.na(sugar_concentration)) %>%
  mutate(sugar_concentration = as.numeric(sugar_concentration)) %>%
  group_by(Family) %>%
...
# Match only valid families
valid_families <- sugar_data$Family

# Step 2: Match family names to Open Tree of Life (OTL) identifiers
matched_names <- tnrs_match_names(names = valid_families)
...
# Step 3: Generate the phylogenetic tree using valid IDs
tree <- tol_induced_subtree(ott_ids = valid_matches$ott_id)
tree$tip.label <- valid_matches$unique_name  # Update tip labels with family names

# Step 4: Prepare heatmap data
# Merge sugar data with the tree tip labels
heatmap_data <- sugar_data %>%
  filter(Family %in% tree$tip.label) %>%
  rename(label = Family) %>%
  select(label, mean_sugar_concentration)

# Ensure order matches the tree
heatmap_data <- tibble(label = tree$tip.label) %>%
  left_join(heatmap_data, by = "label")

# Step 5: Plot the phylogenetic tree with heatmap
p <- ggtree(tree, size = 0.7) +
  geom_tiplab(size = 3, align = TRUE, hjust = -0.1, linesize = 0.5) +
  labs(title = "Phylogenetic Tree with Sugar Concentration Heatmap") +
  theme_tree2()

# Add heatmap of sugar concentrations
gheatmap(p, heatmap_data %>% select(mean_sugar_concentration),
         offset = 1, width = 0.5, colnames = FALSE) +
