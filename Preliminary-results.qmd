...
# Create a named vector of adjusted p-values with comparisons as names
p_values_named <- setNames(dunn_result_df$P.adj, dunn_result_df$Comparison)

# Generate significance groups using multcompView
significance_groups <- multcompView::multcompLetters(p_values_named)$Letters

# Map the significance groups back to plant types
# Extract unique types and create a significance label mapping
type_mapping <- data.frame(
  type = unique(bee_data$type),
  group = sapply(unique(bee_data$type), function(t) {
    # Identify comparisons involving this type
    comparisons <- names(significance_groups)
    relevant_groups <- significance_groups[grep(t, comparisons)]
    paste(unique(relevant_groups), collapse = "")
  })
)
...
ggplot(bee_data, aes(x = type, y = sugar_concentration, fill = type)) +
  ...
  geom_text(
    data = type_mapping,
    aes(x = type, y = max(bee_data$sugar_concentration) + 5, label = group),
    inherit.aes = FALSE,
    size = 5,
    fontface = "bold"
  ) +
